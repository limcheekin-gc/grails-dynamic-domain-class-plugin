...
class HibernateSupport {
	static final Log LOG = LogFactory.getLog(HibernateSupport)
	...
	private static addTransactionalMethods(GrailsDomainClass dc, GrailsApplication application, ApplicationContext ctx) {
		...
		metaClass.static.withSession = { Closure callable ->
			new HibernateTemplate(sessionFactory).execute({ session -> callable(session) } as HibernateCallback)
		}
	...
	private static initiateValidatorBean(domainClass, ctx) {
		def domainClassValidatorName = "${domainClass.fullName}Validator"
		def beans = beans {
			"${domainClassValidatorName}"(HibernateDomainClassValidator) {
				messageSource = ref("messageSource")
				domainClass = ref("${dc.fullName}DomainClass")
				grailsApplication = ref("grailsApplication", true)
			}
		}
		ctx.registerBeanDefinition(domainClassValidatorName,
				beans.getBeanDefinition(domainClassValidatorName))
	}
}
